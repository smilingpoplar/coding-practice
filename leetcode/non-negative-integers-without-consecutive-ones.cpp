...
public:
    int findIntegers(int num) {
        ...
        // 累加[lastPrefix, prefix)间无连续1s的个数。比如num=23 (10111)，要累加统计
        // 二进制区间[0,10000), [10000,10000), [10000,10100), [10100,10110), [10110,10111)。
        // 具体怎么统计区间[lastPrefix, prefix)呢？
        // 若num第i位为1，区间的第i位为0、bit[i-1..0]位任意，贡献f[i]个的无连续1s的数；
        // 若num第i位为0，prefix==lastPrefix，区间为空。
        int count = 0;
        int prebit = 0; // 初始是符号位
        for (int i = N - 2; i >= 0; i--) {
            if (num & (1 << i)) { // 第i位为1
