...
public:
    string smallestGoodBase(string n) {
        ...
        // 要基数b尽量小，则位数x尽量大。b=[2..n-1]，b=2时n=2^x-1，x=lg(n+1)；b=n-1时x=2，故x=[2..lg(n+1)]
        // 由二项式定理(b+1)^(x-1) > b^0+b^1+b^2+...+b^(x-1)=n，又n > b^(x-1)，故 b+1 > pow(n,1/(x-1)) > b
        // b = floor( pow(n,1/(x-1)) )
        long long num = stoll(n);
        for (int x = log2(num + 1); x >= 2; x--) {
            int b = pow(num, 1.0 /(x - 1));
            if (calsSum(x, b) == num) return to_string(b);
        }
        return to_string(num - 1);
    }

    // 对特定x和b计算等比级数n。不能直接用等比公式，因为pow()浮点计算有精度问题！
    long long calsSum(int x, int b) {
