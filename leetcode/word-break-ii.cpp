...
public:
    vector<string> wordBreak(string s, unordered_set<string>& dict) {
        ...
        return generateSentences(s, split);
    }
private:
    vector<string> generateSentences(const string &s, const vector<vector<bool>> &split) {
        vector<string> result;
        vector<string> path;
        generateSentences(s.size(), s, split, path, result);
        return result;
    }
    
    // 为s[0,i)生成句子
    void generateSentences(size_t i, const string &s, const vector<vector<bool>> &split, vector<string> &path, vector<string> &result) {
        if (i == 0) { // 终止条件
        ...
        for (size_t j = 0; j < i; j++) {
            if (split[i][j]) {
                ...
                generateSentences(j, s, split, path, result);
                path.pop_back();
            }
        }
    }
