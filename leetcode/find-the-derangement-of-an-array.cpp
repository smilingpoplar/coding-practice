...
public:
    int findDerangement(int n) {
        ...
        // 考虑元素i换不换到位置1：如果换到位置1，元素1和i的位置确定，剩下n-2个元素的子问题dp[n-2]；如果不换到位置1，
        // 对元素i在除位置i外的n-1个位置中有个禁止位置（位置1），其他元素在除位置i外的n-1个位置中有个禁止位置（现位置），
        // 这就是n-1个元素的子问题dp[n-1]。所以dp[n]=(n-1)(dp[n-2]+dp[n-1])
