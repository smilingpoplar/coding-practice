...
public:
    double findMedianSortedArrays(vector<int> &a, vector<int> &b) {
        ...
        // 先找第k=(M+N+1)/2大的数，k从1开始计数。基本想法是 任意a[i]都可以在O(1)时间内判断它是不是第k大的数
        // 因为若a[i]是第k大的数，它要刚好大于k-1个数，而a[i]已经大于a中它的前i个数，只要它再刚好大于b中前j=k-1-i个数，
        // 即 b[j-1]<=a[i]<=b[j]，a[i]就是第k大的数；同理，a[i-1]<=b[j]<=a[i]，b[j]就是第k大的数
        
        // 对a做二分搜索，不变式：第k大的数x a[l]<=x<=a[r] && l<=r
        // 若a[i]<b[j-1]，a[i]最多大于b中的b[0,j-2]，最多大于总共i+j-1=k-2个数，应继续在a[i+1,M-1]中找
        // 若a[i]>=b[j-1]，a[i]至少大于b中的b[0,j-1]，至少大于总共i+j=k-1个数，应继续在a[0,i]中找
        ...
        // 二分搜索，为避免-1<=j-1<=N-2的越界问题，改搜第k+1大的数！（重点2）
        // 这时j=k-i，不变式：第k+1大的数y a[l]<=y<=a[r] && l<=r
        ...
        // 为表示第k+1大的数y不在a中的情况，引入假想的哨兵 a[M]>=y（重点3）
        //（程序并不实际访问到a[M]，因为i<r==M；若选哨兵a[-1]，i>=l==-1会使a[-1]越界）
        int k = (M + N + 1) / 2;
        int l = 0;
        int r = M; // a[M]是哨兵
        while (l < r) {
        ...
        // 最终l==r，l指向第k+1大的数（在a中的话）或a[M]（不在a中的话）
        // 所以，第k+1大的数是a[l]（在a中的话）或b[k-l]（不在a中的话）
